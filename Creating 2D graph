...
class CreateGraph:
    ...
    def plotting(self, year1, year2, price1, price2, model1, model2, line, font):
        ...
        plt.scatter(year2, price2, label='Good quality', color='#00ff99')
        plt.scatter(year1,price1, label='Low quality', color='#ff00ff')
        plt.plot(line, model1(line), color='#00ff99')
        plt.plot(line, model2(line), color='#ff00ff')
        ax.tick_params(axis='x', colors='white')    #setting up X-axis tick color to red
        ax.tick_params(axis='y', colors='white')
        plt.xlabel('Year', **font)
        plt.ylabel('GBP/£', **font)
        plt.title("The Price Comparison Of Different Muye Cup's Quality", **font)
        plt.legend(loc="upper left")
        plt.show()

if __name__ == '__main__':
    mygraph = CreateGraph(yr1, yr2, p1, p2, mymodel1, mymodel2, myline, font)
    mygraph.plotting(yr1, yr2, p1, p2, mymodel1, mymodel2, myline, font)
